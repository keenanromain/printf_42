...
static int  dispatch_unsigned_numbers(va_list *args, t_flag *f, int len)
{
	if (f->spec == 'o' || f->spec == 'O')
	{
		if (f->spec == 'O')
			f->mod = l;
		f->spec = 'o';
		len += print_octal(args, f, get_unsigned(args, f));
	}
	else if (f->spec == 'u' || f->spec == 'U')
	{
		if (f->spec == 'U')
			f->mod = l;
		f->spec = 'u';
		len += print_uint(args, f,  get_unsigned(args, f));
	}
	else
		len += print_hex(args, f, get_unsigned(args, f));
	return (len);
}
...
int		dispatch_numbers(va_list *args, t_flag *f, int len)
{
	if (f->spec == 'd' || f->spec == 'D' || f->spec == 'i')
	{
		if (f->spec == 'D')
			f->mod = l;
		f->spec = 'd';
		len += print_int(args, f, get_signed(args, f));
	}
	else
